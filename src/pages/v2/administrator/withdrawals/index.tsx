...
export default function WithdrawalsPage() {
  ...
  const fetchWithdrawals = async (page: number = 1) => {
    try {
      ...
      const response = await fetch(`https://api.raspetudo.online/v1/api/admin/withdrawals?page=${page}&limit=20`, {
        headers: {
  ...
  const handleApprove = async (withdrawalId: string) => {
    try {
      ...
      const response = await fetch(`https://api.raspetudo.online/v1/api/admin/withdrawals/${withdrawalId}/approve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleReject = async (withdrawalId: string) => {
    try {
      ...
      const response = await fetch(`https://api.raspetudo.online/v1/api/admin/withdrawals/${withdrawalId}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
